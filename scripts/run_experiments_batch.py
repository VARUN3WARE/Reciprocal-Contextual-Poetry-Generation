...
def run_single(seed, num_prompts, quick, out_path, device=None, model_dir=None, lora_dir=None):
    ...
    if lora_dir:
        cmd.extend(['--lora-dir', str(lora_dir)])
...
def main():
    ...
    # Accept space-separated lists (nargs='+') or comma-separated strings
    parser.add_argument('--seeds', nargs='+', default=['42'], help='Seeds (space separated or comma-separated), e.g. 42 7 123 OR "42,7,123"')
    parser.add_argument('--num-prompts', nargs='+', default=['30'], help='Prompt counts (space separated or comma-separated), e.g. 30 60 OR "30,60"')
    ...
    # Forwarding options to the underlying run_experiments script
    parser.add_argument('--device', type=str, default=None, help='Device to pass to run_experiments (cuda|cpu)')
    parser.add_argument('--model-dir', type=str, default=None, help='Optional path to a local finetuned model directory to pass to run_experiments')
    parser.add_argument('--lora-dir', type=str, default=None, help='Optional path to a PEFT/LoRA adapter directory to pass to run_experiments')
    args = parser.parse_args()

    # Normalize seeds and num-prompts: allow space-separated tokens or comma-separated strings
    seeds = []
